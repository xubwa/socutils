...
def from_dhf(mf, ncore, nact, filename = 'FCIDUMP', tol=1e-10, intor='int2e_spinor'):
    ...
    dm = mf.make_rdm1(mo_occ = core_occ)
    core_energy = scf.hf.energy_elec(mf, dm=dm)
    nuc = mf.energy_nuc() + core_energy[0]
    print(nuc, core_energy, mf.energy_nuc())
    from_integrals(filename = filename, h1e=h1e, h2e=eri, nmo=nact, nelec=sum(mol.nelec)-ncore+nNeg, nuc=nuc)
    return
