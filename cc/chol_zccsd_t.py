...
def kernel(cc, eris, t1=None, t2=None, verbose=logger.INFO):
    ...
    ## Cholesky-direct: rebuild the <bc||ei> slice from the DF factors per (b,c)
    cholVV = numpy.asarray(eris.Lvv).transpose(1, 0, 2)   # (nvir, naux, nvir)
    cholVO = numpy.asarray(eris.Lvo).transpose(1, 0, 2)   # (nvir, naux, nocc)
    ...
    t1T = t1.T
    def get_wv(a, b, c):
        # <bc||ei> = (be|ci) - (ce|bi) rebuilt from the low-rank factors
